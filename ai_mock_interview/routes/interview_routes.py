...
def record_violation():
    ...
    if violation_type == "multiple_faces":
        ...
        if SESSION["no_face_violations"] >= 2:
            SESSION["interview_terminated"] = True
            SESSION["termination_reason"] = "No face detected more than twice"
...
def monitor():
    ...
    if faces != 1:
        ...
        if SESSION["proctoring_issue_count"] >= 5:
            ...
            terminate = True
            reason = "Interview terminated due to repeated face detection issues."
    else:
        # Reset on good frame.
