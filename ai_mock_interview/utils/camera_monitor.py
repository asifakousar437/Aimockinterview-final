...
def detect_faces(base64_image):
    try:
        ...
        # Enhanced face detection with multiple scales for better detection
        faces1 = face_cascade.detectMultiScale(gray, 1.1, 3, minSize=(30, 30))
        faces2 = face_cascade.detectMultiScale(gray, 1.2, 4, minSize=(25, 25))
        faces3 = face_cascade.detectMultiScale(gray, 1.3, 5, minSize=(20, 20))
        
        # Combine results and remove duplicates
        all_faces = list(faces1) + list(faces2) + list(faces3)
        
        # Simple non-maximum suppression to avoid duplicates
        unique_faces = []
        for face in all_faces:
            ...
            for unique_face in unique_faces:
                ...
                if x_overlap > 0.5 * min(w1, w2) and y_overlap > 0.5 * min(h1, h2):
                    is_duplicate = True
                    break
